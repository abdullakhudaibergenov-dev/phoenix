export const translations = {
  en: {
    langLabel: 'EN',
    nav: {
      about: 'About Us',
      services: 'Services',
      references: 'References',
      contacts: 'Contacts',
    },
    hero: {
      eyebrow: 'Premium Corporate Solutions',
      title: 'Elevate Your Business with Precision and Leadership',
      subtitle: 'Phoenix delivers executive consulting and HR transformation services designed for sustainable performance.',
      primaryCta: 'Explore Services',
      secondaryCta: 'Book a Consultation',
    },
    features: {
      consulting: { title: 'Consulting', description: 'Focused advisory support to optimize operations and growth.' },
      insight: { title: 'Insight', description: 'Data-backed analysis that helps teams make strategic decisions.' },
      strategy: { title: 'Strategy', description: 'Clear execution plans aligned with your business priorities.' },
    },
    homeStats: {
      eyebrow: 'Impact Metrics',
      title: 'Measured outcomes behind our consulting work',
      items: [
        { label: 'Years of expertise', value: 20, suffix: '+' },
        { label: 'Corporate clients served', value: 1000, suffix: '+' },
        { label: 'Projects delivered', value: 300, suffix: '+' },
        { label: 'Industry awards', value: 64, suffix: '' },
      ],
    },
    social: {
      eyebrow: 'Social',
      title: 'Stay connected with Phoenix',
      subtitle: 'Explore our latest insights and community highlights across our social channels.',
      followText: 'Follow Us →',
      linkedin: {
        label: 'LinkedIn',
        title: 'Join our professional network',
      },
      instagram: {
        label: 'Instagram',
        title: 'Discover our community',
      },
    },
    services: {
      sectionEyebrow: 'Services',
      sectionTitle: 'Tailored HR and Business Services',
      sectionSubtitle: 'Our modular service portfolio adapts to your team, stage, and strategic objectives.',
      learnMore: 'Learn More →',
      recruitment: {
        title: 'Recruitment',
        description: 'End-to-end hiring solutions to connect you with top talent quickly.',
      },
      hrSolutions: {
        title: 'HR Solutions',
        description: 'Flexible HR operations, compliance support, and policy structuring.',
      },
      talentMapping: {
        title: 'Talent Mapping',
        description: 'Build strategic workforce visibility with market and role intelligence.',
      },
      leadershipDevelopment: {
        title: 'Leadership Development',
        description: 'Equip managers with coaching, communication, and growth frameworks.',
      },
      workforcePlanning: {
        title: 'Workforce Planning',
        description: 'Forecast and design resilient teams to meet future business demand.',
      },
      employerBranding: {
        title: 'Employer Branding',
        description: 'Strengthen your brand perception to attract and retain quality talent.',
      },
    },
    references: {
      eyebrow: 'References',
      title: 'Trusted by Growing Organizations',
      subtitle: 'Selected collaborations that reflect our quality and impact.',
      badge: 'Case Study',
      overview: 'Overview',
      viewCaseStudy: 'View case study →',
      whatWeDidLabel: 'What we did',
      challengesLabel: 'Challenges',
      solutionsLabel: 'Solutions',
      resultsLabel: 'Results',
      companyLabel: 'Company',
      testimonialLabel: 'Client Testimonial',
      enterpriseHiring: {
        title: 'Enterprise Hiring Transformation',
        description: 'Delivered a 35% faster recruitment cycle for a regional technology enterprise.',
      },
      hrModernization: {
        title: 'HR Process Modernization',
        description: 'Redesigned HR workflows for a logistics brand, improving retention by 20%.',
      },
      leadershipRollout: {
        title: 'Leadership Program Rollout',
        description: 'Launched a leadership track across 4 offices with measurable engagement growth.',
      },
      workforceInitiative: {
        title: 'Strategic Workforce Initiative',
        description: 'Built a phased workforce model that aligned hiring plans with expansion targets.',
      },
    },
    contact: {
      eyebrow: 'Contact',
      title: 'Let’s Build Something Great Together',
      subtitle: 'Reach out to discuss your goals and get a customized solution plan.',
      infoTitle: 'Contact Information',
      hours: 'Mon - Fri: 09:00 - 18:00',
      form: {
        name: 'Name',
        email: 'Email',
        phone: 'Phone',
        message: 'Message',
        send: 'Send Message',
      },
    },
    aboutPage: {
      heroTitle: 'About Us',
      heroSubtitle: 'A premium corporate profile built around identity, purpose, and long-term direction.',
      introEyebrow: 'Who We Are',
      introTitle: 'We Build High-Trust Organizations Through Human-Centered Strategy',
      introText:
        'Phoenix combines strategic advisory and execution-focused talent solutions to help modern companies scale with confidence. Our approach aligns leadership, culture, and workforce architecture into one coherent growth system.',
      stats: [
        { label: 'Years of advisory excellence', value: 12, suffix: '+' },
        { label: 'Corporate transformation programs', value: 180, suffix: '+' },
        { label: 'Average retention improvement', value: 22, suffix: '%' },
      ],
      phoenixTitle: 'Phoenix',
      phoenixText:
        'Phoenix stands for disciplined reinvention. We help organizations evolve without losing clarity of identity. Every engagement starts with understanding what makes your company distinct, then shaping systems that protect that advantage as you grow.',
      missionTitle: 'Mission',
      missionText:
        'Our mission is to turn people strategy into measurable business performance. We design practical talent and HR frameworks that improve execution speed, leadership quality, and employee commitment.',
      visionTitle: 'Vision',
      visionText:
        'Our vision is to become the trusted benchmark for premium corporate advisory in growth markets, setting the standard for strategic clarity, ethical leadership, and sustainable workforce transformation.',
      ctaTitle: 'Let’s shape your next chapter with purpose.',
      ctaButton: 'Contact Phoenix',
    },
    footer: {
      quickLinks: 'Quick Links',
      contact: 'Contact',
      description: 'Premium corporate solutions for people, process, and growth.',
      rights: 'All rights reserved.',
    },
  },
  ru: {
    langLabel: 'RU',
    nav: {
      about: 'О нас',
      services: 'Услуги',
      references: 'Кейсы',
      contacts: 'Контакты',
    },
    hero: {
      eyebrow: 'Премиальные корпоративные решения',
      title: 'Развивайте бизнес с точностью и лидерством',
      subtitle: 'Phoenix предоставляет стратегический консалтинг и HR-трансформацию для устойчивого роста компании.',
      primaryCta: 'Смотреть услуги',
      secondaryCta: 'Запросить консультацию',
    },
    features: {
      consulting: { title: 'Консалтинг', description: 'Фокусная экспертная поддержка для оптимизации процессов и роста.' },
      insight: { title: 'Аналитика', description: 'Решения на основе данных для уверенных стратегических шагов.' },
      strategy: { title: 'Стратегия', description: 'Четкие планы реализации, привязанные к бизнес-приоритетам.' },
    },
    homeStats: {
      eyebrow: 'Ключевые метрики',
      title: 'Измеримые результаты нашей консалтинговой работы',
      items: [
        { label: 'Лет экспертизы', value: 20, suffix: '+' },
        { label: 'Корпоративных клиентов', value: 1000, suffix: '+' },
        { label: 'Реализованных проектов', value: 300, suffix: '+' },
        { label: 'Профессиональных наград', value: 64, suffix: '' },
      ],
    },
    social: {
      eyebrow: 'Соцсети',
      title: 'Оставайтесь на связи с Phoenix',
      subtitle: 'Следите за нашими экспертными материалами и жизнью сообщества в социальных сетях.',
      followText: 'Подписаться →',
      linkedin: {
        label: 'LinkedIn',
        title: 'Присоединяйтесь к нашей профессиональной сети',
      },
      instagram: {
        label: 'Instagram',
        title: 'Откройте наше сообщество',
      },
    },
    services: {
      sectionEyebrow: 'Услуги',
      sectionTitle: 'Индивидуальные HR и бизнес-решения',
      sectionSubtitle: 'Наш сервисный портфель адаптируется под вашу команду, этап развития и цели.',
      learnMore: 'Подробнее →',
      recruitment: {
        title: 'Рекрутмент',
        description: 'Полный цикл подбора для быстрого привлечения сильных специалистов.',
      },
      hrSolutions: {
        title: 'HR-решения',
        description: 'Гибкая HR-операционка, комплаенс и построение внутренних политик.',
      },
      talentMapping: {
        title: 'Карта талантов',
        description: 'Стратегическая прозрачность рынка труда и ключевых ролей.',
      },
      leadershipDevelopment: {
        title: 'Развитие лидеров',
        description: 'Развитие управленцев через коучинг, коммуникацию и лидерские практики.',
      },
      workforcePlanning: {
        title: 'Планирование команды',
        description: 'Прогнозирование и проектирование устойчивой структуры персонала.',
      },
      employerBranding: {
        title: 'Бренд работодателя',
        description: 'Усиление репутации компании для привлечения и удержания талантов.',
      },
    },
    references: {
      eyebrow: 'Кейсы',
      title: 'Нам доверяют растущие компании',
      subtitle: 'Выбранные проекты, демонстрирующие качество и результат.',
      badge: 'Кейс',
      overview: 'Обзор',
      viewCaseStudy: 'Смотреть кейс →',
      whatWeDidLabel: 'Что мы сделали',
      challengesLabel: 'Проблемы',
      solutionsLabel: 'Решения',
      resultsLabel: 'Результаты',
      companyLabel: 'Компания',
      testimonialLabel: 'Отзыв клиента',
      enterpriseHiring: {
        title: 'Трансформация подбора в enterprise',
        description: 'Сократили цикл найма на 35% для региональной технологической компании.',
      },
      hrModernization: {
        title: 'Модернизация HR-процессов',
        description: 'Перезапустили HR-потоки в логистической компании и повысили удержание на 20%.',
      },
      leadershipRollout: {
        title: 'Запуск лидерской программы',
        description: 'Внедрили программу развития руководителей в 4 офисах.',
      },
      workforceInitiative: {
        title: 'Стратегическая инициатива по персоналу',
        description: 'Построили поэтапную модель команды под планы расширения бизнеса.',
      },
    },
    contact: {
      eyebrow: 'Контакты',
      title: 'Давайте создадим сильный результат вместе',
      subtitle: 'Свяжитесь с нами, чтобы обсудить цели и получить индивидуальное решение.',
      infoTitle: 'Контактная информация',
      hours: 'Пн - Пт: 09:00 - 18:00',
      form: {
        name: 'Имя',
        email: 'Email',
        phone: 'Телефон',
        message: 'Сообщение',
        send: 'Отправить',
      },
    },
    aboutPage: {
      heroTitle: 'О нас',
      heroSubtitle: 'Премиальный корпоративный профиль, основанный на идентичности, миссии и видении.',
      introEyebrow: 'Кто мы',
      introTitle: 'Мы строим организации высокого доверия через стратегию, ориентированную на людей',
      introText:
        'Phoenix объединяет стратегический консалтинг и практичные talent-решения, помогая компаниям масштабироваться уверенно. Мы синхронизируем лидерство, культуру и архитектуру команды в единую систему роста.',
      stats: [
        { label: 'Лет экспертизы', value: 12, suffix: '+' },
        { label: 'Программ трансформации', value: 180, suffix: '+' },
        { label: 'Рост удержания в среднем', value: 22, suffix: '%' },
      ],
      phoenixTitle: 'Phoenix',
      phoenixText:
        'Phoenix — это дисциплина обновления. Мы помогаем компаниям развиваться, сохраняя сильную идентичность. Каждый проект начинается с понимания уникальности бизнеса и построения систем, которые усиливают это преимущество.',
      missionTitle: 'Mission',
      missionText:
        'Наша миссия — превращать HR-стратегию в измеримую бизнес-эффективность. Мы создаем рабочие модели управления талантами, которые повышают скорость исполнения, качество лидерства и вовлеченность.',
      visionTitle: 'Vision',
      visionText:
        'Наше видение — стать эталоном премиального корпоративного консалтинга на растущих рынках, задавая стандарт стратегической ясности, этичного лидерства и устойчивой трансформации команд.',
      ctaTitle: 'Сформируем ваш следующий этап развития вместе.',
      ctaButton: 'Связаться с Phoenix',
    },
    footer: {
      quickLinks: 'Быстрые ссылки',
      contact: 'Контакты',
      description: 'Премиальные корпоративные решения для людей, процессов и роста.',
      rights: 'Все права защищены.',
    },
  },
};
